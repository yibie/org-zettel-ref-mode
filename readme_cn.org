...
org-zettel-ref-mode 现在支持将概览文件中的高亮从旧的标题（headline）格式转换为新的列表（list）格式。

- **自动转换提示**：当您打开一个包含旧标题格式高亮的概览文件，并且您的 `org-zettel-ref-highlight-format` 配置为 `'list'` 或 `'description'` 时，系统将自动提示您是否进行转换。
- **手动转换**：您也可以在任何 Org 概览文件中运行 =M-x org-zettel-ref-convert-highlights-to-list= 命令，手动将当前缓冲区中的所有标题格式高亮转换为列表格式。

注意：转换操作是不可逆的，请在执行前确认。

* 可调用指令列表

以下是 org-zettel-ref-mode 提供的主要可调用指令：

- =M-x org-zettel-ref-convert-highlights-to-list=: 将当前缓冲区中的高亮从标题格式转换为列表格式
...
- =setq org-zettel-ref-highlight-format 'headline=: 设置概览文件中高亮的显示格式。
  - ='headline=: 使用 Org 标题（*，**，***）来组织高亮。这是传统格式。
  - ='list=: 使用有序列表（-，+，1.）来显示高亮。
