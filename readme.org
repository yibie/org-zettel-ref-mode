...
*** Highlight Format Conversion

org-zettel-ref-mode now supports converting highlights in overview files from the old headline format to the new list format.

- **Automatic Conversion Prompt**: When you open an overview file containing old headline-formatted highlights, and your `org-zettel-ref-highlight-format` is set to `'list'` or `'description'`, the system will automatically prompt you to convert.
- **Manual Conversion**: You can also run the command =M-x org-zettel-ref-convert-highlights-to-list= in any Org overview file to manually convert all headline-formatted highlights in the current buffer to list format.

Note: The conversion operation is irreversible. Please confirm before proceeding.
...
- `M-x org-zettel-ref-convert-highlights-to-list`: Convert highlights in the current buffer from headline format to list format.
...
- `setq org-zettel-ref-highlight-format 'headline`: Set the display format for highlights in overview files.
  - `'headline`: Uses Org headlines (*, **, ***) to organize highlights. This is the traditional format.
  - `'list`: Uses ordered lists (-, +, 1.) to display highlights.
...
** Changlog
*** 
*** Version 0.6.0 (2025-07-27)
  - Added: Highlight Display Format Options
    - New configuration option: `org-zettel-ref-highlight-format`, allowing users to choose the display format for highlights in overview files (headline, list, description list).
    - Improved: `org-zettel-ref--add-or-update-highlights` function updated to support new highlight formats.
  - Enhanced: Highlight Link Jumping
    - Fixed issues with `hl:` links not jumping correctly; now, highlights in both headline and list formats can correctly jump back to the source file.
  - Added: Highlight Format Conversion Tool
    - New command: `org-zettel-ref-convert-highlights-to-list`, for converting old headline-formatted highlights to the new list format.
    - New feature: Automatic prompt for conversion when old format highlights are detected and the new format is configured.
